<header>
    <div>

            <img style="width: 100px; height: 100px;" src="recursos/imagenes/logo.png">
            <!--Logo-->

            <h1>Pokedex</h1>
            <!--User-->
            <form class="form-inline" action=paginas/sesion.php" method="POST">
                <label class="mb-2 mr-sm-2">Usuario:</label>
                <input class="form-control mb-2 mr-sm-2" type="text" name="usuario" placeholder="usuario" id="usuario">
                <label class="mb-2 mr-sm-2">Contraseña:</label>
                <input class="form-control mb-2 mr-sm-2"  type="password" name="contrasenia" placeholder="clave" id="contrasenia">
                <button type="submit" class="btn btn-primary mb-2">Ingresar</button>
            </form>

    </div>
</header>
